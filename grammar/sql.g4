...
statement
    ...
    | CREATE TABLE (catcatalogName=identifier '.')? tableIdentifier
        ('(' columns=dtColTypeList ')')?
        (COMMENT comment=STRING)?
        (PARTITIONED BY '(' partitionColumnNames=identifierList ')')?
        WITH tablePropertyList                                         #createFlinkTable
...
relationPrimary
    : tableIdentifier (sample | matchRecognize)? tableAlias      #tableName
...
matchRecognize:
      ...
      ( AFTER MATCH
            ( SKIP1 TO NEXT ROW
            | SKIP1 PAST LAST ROW
            | SKIP1 TO FIRST variable
            | SKIP1 TO LAST variable
            | SKIP1 TO variable )
      )?
      PATTERN '(' pattern1 ')'
      ( WITHIN interval )?
      DEFINE variable AS condition1 (',' variable AS condition1 )*
      ')'
    ;
measureColumn:
      expression AS strictIdentifier
    ;
condition1:
    expression
    ;
...
tableIdentifier
    ...
    | LATERAL TABLE '(' functionIdentifier '(' expression (',' expression )* ')' ')'
    | UNNEST '(' expression ')'
    ;

functionIdentifier
    : (db=identifier '.')? j_function=identifier
    ;
...
dtColType
    : dtColIdentifier ('.' dtColIdentifier)* dataType (AS identifier)? (COMMENT STRING)?
    | WATERMARK FOR identifier AS primaryExpression
    | 'PRIMARY' 'KEY' '(' dtColIdentifier (',' dtColIdentifier)* ')' 
    ;
dtColIdentifier
    : identifier
...
nonReserved
    ...
    | REVOKE | GRANT | LOCK | UNLOCK | MSCK | REPAIR | RECOVER | EXPORT | IMPORT | LOAD | COMMENT | ROLE
